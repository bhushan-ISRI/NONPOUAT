...
type Props = ISonaNonPoProps & {
  title?: string;
  listTitle?: string; 
};


//export const NonPoApproval = (props: ISonaNonPoProps) => {
const NonPoApproval: React.FC<Props> = ({
  ...
  const loadItem = async () => {
    ...
    const rows =
      (await spCrudOps.getTopData(
        ...
        { currentSPContext } as ISonaNonPoProps
      )) ?? [];
  ...
  const handleSave = async () => {
    try {
      ...
      await spCrudOps.updateData(listTitle, numericId, data, { currentSPContext } as ISonaNonPoProps);
